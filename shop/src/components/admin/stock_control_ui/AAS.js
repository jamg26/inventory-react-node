        ...
        if (index === editingIndex) {
          return [
            <DatePicker
              format={dateFormat}
              defaultValue={moment(
                result.po_items[0].delivery_due_date,
                dateFormat
              )}
              key={index}
              disabled={editable}
              onChange={(event) => setInput(event, index, "delviery_due_date")}
            />,
          ];
        } else {
          return [
        ...
        if (index === editingIndex) {
          return [
            <InputNumber
              key={index}
              disabled={editable}
              value={result.po_items[0].quantity}
              onChange={(event) => {
                setInputSub(event, index, "quantity");
              }}
            />,
          ];
        } else {
        ...
        if (index === editingIndex) {
          return [
            <InputNumber
              key={index}
              disabled={editable}
              value={result.po_items[0].total}
              onChange={(event) => {
                setInputSub(event, index, "total");
              }}
            />,
          ];
        } else {
              ...
              <Button
                ...
                onClick={() =>
                  onSave(
                    ...
                    row.type,
                    row.status,
                    row.po_items[0].bill_to,
                    row.po_items[0].ship_to,
                    row.po_items[0].quantity,
                    row.po_items[0].delivery_due_date,
                    row.po_items[0].item_cost,
                    row.po_items[0].tax,
                    row.po_items[0].total
                  )
                }
              />
              <Button
    ...
    setfilteredPurchaseOrderData(
      purchaseOrderData.filter(
        (data) =>
          ...
          data.po_items[0].bill_to
            .toLowerCase()
            .includes(search.toLowerCase()) ||
          data.po_items[0].ship_to
            .toLowerCase()
            .includes(search.toLowerCase()) ||
          data.po_items[0].delivery_due_date
            .toLowerCase()
            .includes(search.toLowerCase()) ||
          data.po_items[0].item_cost
            .toString()
            .toLowerCase()
            .includes(search.toLowerCase()) ||
          data.po_items[0].quantity
            .toString()
            .toLowerCase()
            .includes(search.toLowerCase()) ||
          data.po_items[0].tax
          ...
          <Col span={12}>
            <Row span={24}>
              ...
              <CSVLink
                ...
                data={filteredPurchaseOrderData}
              >
