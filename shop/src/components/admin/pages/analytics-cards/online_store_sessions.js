...
function Overview() {
  ...
  const get_abandoned_carts = async () => {
    ...
    const response = await axios.get(
      api_base_url + "/get_login_logs_today",
      {},
      { headers: headers }
    );
    setorders_today(response.data.today);
    setorders_yesterday(response.data.yesterday);
  };
  useEffect(() => {
    let temp = 0;
    let temp_data = [
    ...
    for (let c = 0; c < orders_today.length; c++) {
      ...
      temp = parseFloat(temp) + parseFloat(element.number_of_logins);
      if (temp_data[element._id.hour]) {
        temp_data[element._id.hour].today = element.number_of_logins;
      }
    }
    let temp2 = [];
    for (let c = 0; c < orders_yesterday.length; c++) {
      ...
      if (temp_data[element._id.hour]) {
        temp_data[element._id.hour].yesterday = element.number_of_logins;
      }
    }
    let newcontainer = [];
