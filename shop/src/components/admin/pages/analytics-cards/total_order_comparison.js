...
function Overview() {
  ...
  useEffect(() => {
    ...
    for (let c = 0; c < orders_today.length; c++) {
      ...
      temp = parseFloat(c) + parseFloat(element.number_of_orders);
      if (temp_data[element._id.hour]) {
        temp_data[element._id.hour].today = element.number_of_orders;
      }
    }
    let temp2 = [];
    for (let c = 0; c < orders_yesterday.length; c++) {
      ...
      if (temp_data[element._id.hour]) {
        temp_data[element._id.hour].yesterday = element.number_of_orders;
      }
    }
    let newcontainer = [];
